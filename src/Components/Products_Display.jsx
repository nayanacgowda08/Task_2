...
const Products_Display = ({ selectedCategory }) => {
  ...
  const fetchApi = async () => {
    try {
      const url = selectedCategory 
        ? `${BASE_URL}/user/category/${selectedCategory}` // Fetch products by selected category
        : `${BASE_URL}/user/all`; // Fetch all products
  ...
  useEffect(() => {
    fetchApi(); // Fetch products when component mounts or category changes
  }, [selectedCategory]);
